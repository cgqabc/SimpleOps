    ...
    def add_dynamic_group(self, hosts, groupname, groupvars=None):
        ...
        for host in hosts:
            ...
            if username == 'root':
                ...
                ssh_key = "/home/{user}/.ssh/id_rsa".format(user=username)
        ...
        try:
            ...
            print("ansible添加资产组失败: {ex}".format(ex=ex))
...
class ANSRunner(object):
    ...
    def __initializeData(self, kwargs):
        ...
        Options = namedtuple('Options', ['connection', 'module_path', 'forks', 'timeout',
                                         'remote_user', 'ask_pass', 'private_key_file',
                                         'ssh_common_args', 'ssh_extra_args', 'sftp_extra_args',
                                         'scp_extra_args', 'become', 'become_method',
                                         'become_user', 'ask_value_pass', 'verbosity',
                                         'check', 'listhosts', 'listtasks', 'listtags',
                                         'syntax', 'ask_sudo_pass'])

        self.variable_manager = VariableManager()
        self.loader = DataLoader()
        self.options = Options(connection=kwargs.get('connection', 'smart'),
                               module_path=None, forks=100, timeout=10,
    ...
    def run_model(self, host_list, module_name, module_args):
        ...
        try:
            ...
            if self.redisKey: Redis_pool.lpush(self.redisKey,
        ...
        try:
            ...
            logger.error(msg="run model failed: {err}".format(err=str(err)))
            if self.redisKey: Redis_pool.lpush(self.redisKey, data=err)
    ...
    def run_playbook(self, host_list, playbook_path, extra_vars=dict()):
        ...
        try:
            ...
            executor = PlaybookExecutor(
                playbooks=[playbook_path], inventory=inventory, variable_manager=self.variable_manager,
                loader=self.loader,
...
if __name__ == '__main__':
    ...
    #                 }
    #             }
    # rbt = ANSRunner(resource, redisKey='1')  # 如果带rediskey参数则是直接保存结果和推送redis
