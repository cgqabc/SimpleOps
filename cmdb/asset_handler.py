...
class New_asset():
    ...
    def add_to_new_assets_zone(self):
        defaults = {
            ...
            'cpu_number': self.data.get('cpu_number'),
            'cpu_core': self.data.get('cpu_core'),
...
class ApproveAsset:
    ...
    def _create_asset(self):
        ...
        asset = models.Assets.objects.create(asset_type=self.new_asset.asset_type,
    ...
    def _create_server(self, asset):
        ...
        :return:
        """
        server = models.Server_asset.objects.create(asset=asset,
        ...
        # server.cpu_model = self.new_asset.cpu_model
        # server.cpu_number = self.new_asset.cpu_number
        # server.cpu_core = self.new_asset.cpu_core
        # server.save()
        print('create server ok!')
    ...
    def _create_CPU(self, asset):
        ...
        cpu.cpu_number = self.new_asset.cpu_number
        cpu.cpu_core = self.new_asset.cpu_core
        cpu.save()
...
class UpdateAsset:
    ...
    def _update_assets(self):
        ...
        更新服务器
        """
        self.asset.status = self.report_data.get('status')
        self.asset.business_unit = self.report_data.get('business_unit')
        self.asset.asset_model = self.report_data.get('asset_model')
        self.asset.site = self.report_data.get('site')
        # self.asset.server_asset.cpu_model = self.report_data.get('cpu_model')
        # self.asset.server_asset.cpu_number = self.report_data.get('cpu_number')
        # self.asset.server_asset.cpu_core = self.report_data.get('cpu_core')
        self.asset.manage_ip = self.report_data.get('manage_ip')
        self.asset.manager = self.report_data.get('manager')
        if self.report_data.get('buy_time'):
            self.asset.buy_time = datetime.datetime.strptime(self.report_data.get('buy_time'),'%Y-%m-%d')
        if self.report_data.get('expire_date'):
            self.asset.expire_date = datetime.datetime.strptime(self.report_data.get('expire_date'),'%Y-%m-%d')

        self.asset.save()
        print 'assets update ok'

    def _update_manufacturer(self):
        """
        更新厂商
        """
        m = self.report_data.get('manufacturer', None)
        if m:
            manufacturer_obj, _ = models.Manufacturer.objects.get_or_create(name=m)
        ...
        print('update manufacturer ok!')

    def _update_server(self):
        """
        更新服务器
        """
        self.asset.server_asset.model = self.report_data.get('model')
        self.asset.server_asset.os_type = self.report_data.get('os_type')
        self.asset.server_asset.os_distribution = self.report_data.get('os_distribution')
        self.asset.server_asset.os_release = self.report_data.get('os_release')
        # self.asset.server_asset.cpu_model = self.report_data.get('cpu_model')
        # self.asset.server_asset.cpu_number = self.report_data.get('cpu_number')
        # self.asset.server_asset.cpu_core = self.report_data.get('cpu_core')
        self.asset.server_asset.hosted_on = self.report_data.get('hosted_on')
        self.asset.server_asset.raid_type = self.report_data.get('raid_type')
        self.asset.server_asset.username = self.report_data.get('username')
        self.asset.server_asset.passwd = self.report_data.get('passwd')
        self.asset.server_asset.sudo_passwd = self.report_data.get('sudo_passwd')
        self.asset.server_asset.hostname = self.report_data.get('hostname')
        self.asset.server_asset.keyfile = self.report_data.get('keyfile')
        self.asset.server_asset.port = self.report_data.get('port')
        self.asset.server_asset.disk_total = self.report_data.get('disk_total')
        self.asset.server_asset.ram_total = self.report_data.get('ram_total')
        self.asset.server_asset.swap = self.report_data.get('swap')

        self.asset.server_asset.save()
        print 'server ok'
    ...
    def _update_CPU(self):
        ...
        :return:
        """
        c, _ = models.CPU.objects.get_or_create(asset=self.asset)
        c.cpu_model = self.report_data.get('cpu_model')
        c.cpu_number = self.report_data.get('cpu_number')
        c.cpu_core = self.report_data.get('cpu_core')
        c.save()
        print 'cpu ok'
    ...
    def _update_RAM(self):
        ...
        """
        # 获取已有内存信息，并转成字典格式
        ram = self.report_data.get('ram')
        if not ram:
            return
        old_rams = models.Ram_asset.objects.filter(asset=self.asset)
        old_rams_dict = dict()
        if old_rams:
            for ram in old_rams:
        ...
        if new_rams_dict:
            for key in new_rams_dict:
                ...
                models.Ram_asset.objects.update_or_create(asset=self.asset, slot=key, defaults=defaults)
        print 'ram ok'
    ...
    def _update_disk(self):
        ...
        更新硬盘信息。类似更新内存。
        """
        disk = self.report_data.get('physical_disk_driver')
        if not disk:
            return
        old_disks = models.Disk_asset.objects.filter(asset=self.asset)
        old_disks_dict = dict()
        if old_disks:
        ...
        if new_disks_dict:
            for key in new_disks_dict:
                ...
                models.Disk_asset.objects.update_or_create(asset=self.asset, sn=key, defaults=defaults)
        print 'disk ok'
